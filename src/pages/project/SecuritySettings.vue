	...
	<div>
		...
		<div class="px-6 py-5 space-y-6">
			...
			<div class="space-y-4">
				...
				<RadioGroup
					v-model="securityOptions.rateLimiting"
					class="space-y-3"
				>
					<RadioGroupOption
						value="none"
					...
					>
						<div class="flex items-start cursor-pointer">
							<div class="flex items-center h-5 mt-0.5">
								<div
									class="relative flex items-center justify-center h-4 w-4 border rounded-full"
									:class="checked ? 'border-indigo-500' : 'border-gray-300'"
								>
									<div
										v-if="checked"
										class="absolute h-2 w-2 rounded-full bg-indigo-600"
									></div>
								</div>
							</div>
						...
						<div class="flex items-start cursor-pointer">
							...
							<div class="ml-3 text-sm">
								<label class="font-medium text-gray-700"
									>Стандартный (100 событий/минуту)</label
								>
								<p class="text-gray-500">
									Рекомендуемый лимит для большинства проектов
								</p>
							</div>
						</div>
					</RadioGroupOption>
								...
								<div class="flex flex-col sm:flex-row sm:items-center gap-3">
									...
									<div class="flex items-center space-x-2">
										...
										<span class="text-gray-500 whitespace-nowrap"
											>событий в минуту</span
										>
									</div>
								</div>
								<p
									v-if="checked && securityOptions.customRateLimit > 1000"
									class="mt-1 text-yellow-600 text-xs"
								>
			...
			<div class="flex justify-end">
				<SubmitButton class="cursor-pointer"> Сохранить </SubmitButton>
			</div>
		</div>
	</div>
</template>
...
watch(
	() => securityOptions.value.rateLimiting,
	(newVal) => {
		if (newVal !== 'custom') {
